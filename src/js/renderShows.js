...
function renderShows() {
  ...
  for (const show of showsList) {
    ...
    console.log(typeof show.mal_id);

    if (seriesInFav !== -1) {
      //seriesInFav.classList.add('fav');

      html += `<li class="js-show-selected js-show-selectedfav fav show" id="${show.mal_id}">`;
    } else {
      html += `<li class="js-show-selected js-show-selectedfav show" id="${show.mal_id}">`;
    }

    let urlImg = `<img src="${show.images.jpg.image_url}" alt="${show.title}">`;
    if (
      urlImg ===
      `<img src="https://cdn.myanimelist.net/img/sp/icon/apple-touch-icon-256.png" alt="${show.title}">`
    ) {
